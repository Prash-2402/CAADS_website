 ...
 */
// Root domain — any subdomain of this is also accepted.
// e.g. xy.z@btech.christuniversity.in, xy.z@mca.christuniversity.in, etc.
...
export function isAllowedEmail(email: string): boolean {
  ...
  return ALLOWED_EMAIL_DOMAINS.some(
    (domain) =>
      // Direct domain:    user@christuniversity.in
      lower.endsWith(`@${domain}`) ||
      // Any subdomain:    user@btech.christuniversity.in
      lower.endsWith(`.${domain}`),
  );
}
